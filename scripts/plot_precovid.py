...
data = (
    pd
    .read_csv("results/precovid_under_payment_results.csv")
    .merge(
        pd.read_csv("results/precovid_over_payment_results.csv"),
        how="outer",
        on=["year", "state"]
    )
)
...
def plot_state(state1, state2):
    ...
    for y, c in zip([2017, 2018, 2019], ["purple_light", "purple", "purple_dark"]):
        plt.scatter(
    ...
    for y, c in zip([2017, 2018, 2019], ["gold_light", "gold", "gold_dark"]):
        plt.scatter(
    ...
    plt.title("Predicted SNAP error rate from state policies, pre-COVID (2017-2019)", fontsize=16, loc="left")
    plt.legend(title=False)
    rsquared(data["y_true"], data["y_pred"])
    remove_spines(plt.gca())
    plt.tight_layout()
    plt.savefig("results/precovid_snap_error_rate.pdf")
    plt.savefig("results/precovid_snap_error_rate.png")

print("Plotting pre-COVID")
